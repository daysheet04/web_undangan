...
export default function AdminPage() {
  ...
  const sidebarNavigation = <nav className="admin-sidebar-nav" aria-label="Menu admin" onClick={() => { setShowNotifications(false); setMobileTabOpen(false); document.body.classList.remove('fee-mobile-menu-open'); }} onPointerUp={(event) => { const label = event.target.closest('button')?.textContent?.trim(); if (label === 'Kelola fee') setActiveView('fee'); if (label === 'Laporan fee') setActiveView('fee-report'); }}><button className={activeView === 'report' ? 'is-active' : ''} type="button" onClick={() => setActiveView('report')}>Laporan</button><button className={activeView === 'fee' ? 'is-active' : ''} type="button" onClick={() => setActiveView('fee')}>Kelola fee</button><button className={activeView === 'fee-report' ? 'is-active' : ''} type="button" onClick={() => setActiveView('fee-report')}>Laporan fee</button><button className={activeView === 'setting' ? 'is-active' : ''} type="button" onClick={() => setActiveView('setting')}>Setting harga</button><button className={activeView === 'editor' ? 'is-active' : ''} type="button" onClick={() => setActiveView('editor')}>Editor customer</button><button className="admin-sidebar-logout" type="button" onClick={logout}>Keluar</button></nav>;
